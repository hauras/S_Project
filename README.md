...
플레이어가 여러 캐릭터를 실시간으로 교체(태그)하며 싸우는 시스템을 구현했습니다. 교체 플레이 자체에 전략적 재미를 부여하기 위해 **'시너지 어택(Synergy Attack)'** 기능을 더했습니다.
...
* 전사와 마법사의 스킬 연계를 하드코딩으로 처리하지 않고, 언리얼 GAS의 **Gameplay Tag**를 적극 활용했습니다. [[📄시너지 판정 로직]](링크)
* 전사로 스킬을 맞추면 대상에게 '시너지 표식' 태그를 남기고, 마법사로 교체해 공격할 때 대상에게 해당 태그가 있는지 검사하여 폭발 데미지(Effect)를 줍니다.
* 복잡한 조건 처리를 태그 기반으로 분리하여, 향후 새로운 캐릭터나 조합이 추가되어도 쉽게 확장할 수 있는 유연한 구조를 만들었습니다.
